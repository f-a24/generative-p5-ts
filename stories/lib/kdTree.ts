...
class KdNode {
  ...
  constructor(obj: rgbObj, dimension: number, parent: KdNode) {
    this.obj = obj;
    this.left = null;
    this.right = null;
    this.parent = parent;
...
class BinaryHeap {
  content: [node: KdNode, distance: number][];
  scoreFunction: (el: [node: KdNode, distance: number]) => number;

  constructor(scoreFunction: (el: [node: KdNode, distance: number]) => number) {
    this.content = [];
    this.scoreFunction = scoreFunction;
  }
  push(element: [node: KdNode, distance: number]) {
...
export function kdTree(
  points: rgbObj[],
  metric: (a: rgbObj, b: rgbObj) => number,
  dimensions: ['r', 'g', 'b']
) {
  const self = this;

  const buildTree = (points: rgbObj[], depth: number, parent: KdNode) => {
  ...
  const loadTree = (data: KdNode) => {
    // Just need to restore the `parent` parameter
    self.root = data;

    const restoreParent = (root: KdNode) => {
      if (root.left) {
  ...
  // the `parent` property
  this.toJSON = function (src: KdNode) {
  ...
  this.insert = function (point: rgbObj) {
    const innerSearch = (node: KdNode, parent: KdNode): KdNode => {
  ...
  this.remove = function (point: rgbObj) {
    const nodeSearch = (node: KdNode) => {
      if (node === null) return null;
      if (node.obj === point) node;

      const dimension = dimensions[node.dimension];

      if (point[dimension] < node.obj[dimension]) nodeSearch(node.left);
      else return nodeSearch(node.right);
    };

    const removeNode = (node: KdNode) => {
      const findMin = (node: KdNode, dim: number) => {
        if (node === null) return null;
      ...
      let nextNode: KdNode, nextObj: rgbObj, pDimension: string;
    ...
    const node = nodeSearch(self.root);
  ...
  this.nearest = function (
    ...
    const nearestSearch = (node: KdNode) => {
      const dimension = dimensions[node.dimension];
      const ownDistance = metric(point, node.obj);
      const linearPoint: rgbObj = { name: null, r: null, g: null, b: null };

      let bestChild: KdNode,
      ...
      if (node.right === null) bestChild = node.left;
        ...
        if (point[dimension] < node.obj[dimension]) bestChild = node.left;
  ...
  this.balanceFactor = () => {
    const height = (node: KdNode) =>
      node === null ? 0 : Math.max(height(node.left), height(node.right)) + 1;
    const count = (node: KdNode) =>
      node === null ? 0 : count(node.left) + count(node.right) + 1;
